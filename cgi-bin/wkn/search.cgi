...
##
my($match_prefix_url) = "$wkn::define::cgi_wpath/" .
   &wkn::mode_to_scriptprefix(
...
my($search_root) = "$wkn::define::notes_dir$notes_subpath" ;
my(@dirs) = ( "$search_root" );
...
DIR: while (@dirs)
{
   ...
   if ($found )
   {
      my($auth_path) = $wkn::define::auth_subpath;
      if(auth::check_file_auth( $user, 'r', $wkn::define::auth_subpath, $notes_subpath ))
      {
