...
if(-d $toppath)
{
   ...
   while( @dirs )
   {
      ...
      {
         $link = "wkn_plain.cgi?$encoded_notes_path";
       ...
       {
          $link = "$wkn::define::notes_wpath/$encoded_notes_path";
            $count++;
       }
...
{
   $notes_path =~ m:([^/]*)$:;
   $back_link = "<a href=\"wkn_js.cgi?" . &wkn::url_encode_path($`) ."\">[&lt;-]</a>";
}
else
{
   $notes_name = "Main";
...
${back_link}<a href="wkn_plain.cgi?$notes_path_encoded">$notes_name</a><br>
   <script language="JavaScript">
   <!-- Hide from browsers without js enabled;
   menu.display();
   // End script hiding -->
   ...
   <p>See <a href="wkn_other.cgi">other browse method</a> instead.
   </NOSCRIPT>
</body>
</html>
EOT
